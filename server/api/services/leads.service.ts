...
export class LeadService {
  async create(lead: Lead): Promise<RegistrationResult> {
    const org: PipedriveOrganization = new PipedriveOrganization(
      lead.businessName
    );
    ...
    const person: PipedrivePerson = new PipedrivePerson(
      lead.personName,
      createdOrg.id,
      lead.email,
      lead.phone
    ...
    const createdDeal = await pipedriveApi.createDeal(
      deal,
      createdPerson,
      createdOrg
    );
    const note: PipedriveNote = new PipedriveNote(
      `Zustimmung zur Verarbeitung von personenbezogenen Daten: ${
        lead.consent
      }, 
      Der Kunde ist an Währungen interessiert: ${lead.currencies.join(', ')}`
    );
    if (createdDeal.id) {
      await pipedriveApi.createNote(note, createdDeal.id);
    }
    if (!createdDeal.id) {
