...
export class PipedriveApi {
  ...
  constructor(pipedriveApiOptions: PipedriveApiOptions) {
    ...
    this.axios.interceptors.request.use((config) => {
      config.params = {
        ...config.params,
        api_token: pipedriveApiOptions.apiToken,
      };
      return config;
    });
  }
  ...
  async createNote(note: PipedriveNote, id: PipedriveDeal['id']) {
    const createResult = await this.axios.post('notes', {
      content: note.note.toString(),
      deal_id: id,
    });
    const responseData = createResult.data;
    if (responseData.success) {
      return new PipedriveNote(note.note);
    } else {
      throw new PipedriveException(1000, `General exception`);
    }
  }
